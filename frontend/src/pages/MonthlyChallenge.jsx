...
const MonthlyChallenges = () => {
  ...
  const safeChallenges = Array.isArray(monthchallenges)
    ? monthchallenges
    : monthchallenges?.data?.monthchallenges ||
  ...
  // Ref for hidden file input (Required for click-to-upload)
  ...
  // 🧩 Fetch monthly challenges on load
  useEffect(() => {
  ...
  const handleSubmit = async (index) => {
    ...
    // Note: No need to explicitly clear form data here if the store logic updates
    // the challenge status (c.submitted = true), which will disable the form.
  };
            ...
            <span
              onClick={() => {
                if (safeChallenges.length === 0)
                  generateChallenges(userId, "monthly");
              }}
              ...
              >
                ...
                >
                    ...
                    <div className="text-xs text-gray-400 mb-2">
                      **Challenge:** {c.title || "Monthly Quest"}
                    </div>
                </div>
                    ...
                    >
                        {formData[i]?.imagePreview ? (
                            <div className="relative w-full h-32">
                                ...
                                />
                                {/* 💡 Hide remove button if submitted */}
                                {!c.submitted && (
                                  <button
